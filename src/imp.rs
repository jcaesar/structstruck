...
fn strike_through_attributes(
    ...
    let each_pos = dec_attrs
        .iter()
        .position(|attr| check_crate_attr_no_params(attr, "here_each", ret));
    if let Some(each_pos) = each_pos {
        dec_attrs.remove(each_pos);
    }
    let each_pos = each_pos.unwrap_or(0);
